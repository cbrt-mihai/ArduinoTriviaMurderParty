...
String getTopic(bool x, byte pos) {
  if(x == 0) {
    return topics(newrandom(0, NUMBER_OF_TOPICS));
  }
  else {
    return topics(pos);
  }
...
byte generateQuestionIndex(String topic) {
  if(topic == "gaming") {
    return newrandom(0, NUMBER_OF_QUESTIONS);
  }
  if(topic == "anime") {
    return newrandom(0, NUMBER_OF_QUESTIONS);
  }
...
void generateQuestionData() {
  topicIndex = newrandom(0, NUMBER_OF_TOPICS);
  questionIndex = newrandom(0, NUMBER_OF_QUESTIONS);
}
...
void loop() {
  ...
  if(state == NEW_HIGHSCORE) { // newHighscore
    lcd.setCursor(0, 0);
...
/*
 Is in charge of displaying the options for each Menu Item (Yes/No, +++/-- etc.) and checking
 to see if the joystick was clicked. This function also moves the states.
 */
...
/*
  This function displays all the data regarding the player (HP, score and name)
 and all the data regarding the current question (Question, Answer 1 etc.)
 */
...
/*
  Displays the icon on the 8x8 matrix
 */
...
/*
  Returns the name of the player at the index specified,
 by comparing the specified name with each name in the top 3;
 */
...
/*
    Adds the new highscore in the correct place by comparing
  the current score with the ones in top 3. If it finds one that
  is lower than the current one, it overwrites it;
 */
...
/*
  Resets the highscore by overwriting the top 3 with 3 entries 
 of name None and score 0;
 */
void resetHighscore() {
  uint16_t address = 45;
